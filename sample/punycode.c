...
int main (int argc, char ** argv) {
	...
	puny = (UChar *) strdup (convert_punycode(domain, 0, 1));
	printf ("%s\n", strcmp ((char *) puny, punyc) ? "failed" : "ok");

	stringprep_locale_charset_cache = "EUC-KR";

	oc_test_banner ("convert_punycode decode");
	unpuny = (UChar *) strdup (convert_punycode (punyc, 1, 1));
	printf ("%s\n", strcmp (domain, (char *) unpuny) ? "failed" : "ok");
	...
	l = convert_punycode_r ((CChar *) puny, &unpuny, true, "EUC-KR");
	if ( l == 0 || unpuny == null )
		printf ("failed - mem allocate\n");
	else
		printf ("%s\n", strcmp (domain, (char *) unpuny) ? "failed" : "ok");
